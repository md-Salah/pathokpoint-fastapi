...
async def test_get_my_order_by_id(diff, client: AsyncClient, address_in_db: dict, order_in_db: dict, admin_in_db_with_token: dict):
...
async def test_update_order_status(client: AsyncClient, order_in_db: dict, admin_in_db_with_token: dict):
    ...
    response = await client.patch(f"/order/{order_in_db['id']}", json=payload, headers=admin_in_db_with_token['auth_headers'])
    assert response.status_code == status.HTTP_200_OK
    assert len(response.json()['order_status']) == 2


async def test_update_order_payment_by_admin(client: AsyncClient, order_in_db: dict, payment_gateway_in_db: dict, admin_in_db_with_token: dict):
    payload = {
        'transaction': {
...
async def test_update_order_item_by_admin(client: AsyncClient, order_in_db: dict, book_in_db: dict, admin_in_db_with_token: dict):
