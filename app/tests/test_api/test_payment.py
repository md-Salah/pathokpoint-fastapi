...
@patch("app.library.bkash.grant_token")
async def test_pay_with_bkash_success_callback(grant_token, execute_payment, client: AsyncClient, order_in_db: dict, payment_gateway_in_db: dict):
    grant_token.return_value = 'token'
    ...
    assert res.headers['location'].endswith(
        '/checkout/success?invoice={}'.format(order_in_db['invoice']))
